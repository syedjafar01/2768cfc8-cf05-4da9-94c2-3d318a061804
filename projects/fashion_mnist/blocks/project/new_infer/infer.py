...
@rf.block
class LoadImagesAndInfer:
    __publish__ = True
    __label__ = 'LoadAndInfer'    
    ...
    def run(self):
        ...
        # fit method
        output = model.predict_generator(layers=['argmax','labels'],
                                        test_data=generator,
                                        batch_size=1,
                                        model_load_path="model_save_folder")
        
        for batch in output:
            y_true.extend(list(np.argmax(batch['labels'], axis=1)))
            y_hats.extend(list(batch['argmax']))
            
        df = pd.DataFrame({'y_true':y_true, 'y_hats':y_hats}).to_csv('/home/aios/projectspace/preds.csv', index=False, mode='a')
